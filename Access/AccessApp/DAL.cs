    ...
    public class DAL
    {
        ...
        public static DataSet getProductPerEpiId(string EpiId)
        {
            ...
            */
            ...
            return _db.ExecuteQuery(string.Format("select {0}.EPIID, {0}.OPERATION_DATE, {1}.LAST_NAME, {1}.FIRST_NAME, {2}.LOCALISATION_ID,{3}.NAME, {4}.NAME AS MODELE ,{5}.SERIAL_NUMBER, {0}.STATUS_TO FROM {0} RIGHT OUTER JOIN {1} ON {0}.CONTACT_ID = {1}.ID RIGHT OUTER JOIN {2} ON {0}.ID_LOCALISATION_TO = {2}.ID RIGHT OUTER JOIN {5} ON {0}.EPIID = {5}.EPIID RIGHT OUTER JOIN {3} ON {5}.BRAND_ID = {3}.BRAND_ID RIGHT OUTER JOIN {4} ON {5}.MODEL_ID = {4}.MODEL_ID WHERE {0}.EPIID like :id ORDER BY {0}.OPERATION_DATE DESC",Consts.HISTORIC_TABLE, Consts.CONTACTS_TABLE, Consts.LOCALISATION_TABLE, Consts.BRAND_TABLE, Consts.MODEL_TABLE, Consts.STOCK_TABLE), parameters, values);
        }
