    ...
    public partial class GestionCMDB : System.Web.UI.Page
    {
        ...
        protected void B_apply_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (DDL_status.SelectedValue.ToString() != "OBSOLETE")
                {
                    ...
                    if (DAL.InsertInHistoric(TB_id_resp.Text, DDL_status.SelectedValue.ToString(), CheckEpi(), Consts.ID_LOCALISATION,"0", TB_note.Text.ToString()) && DAL.UpdateStockStatus(CheckEpi(), DDL_status.SelectedValue.ToString()))
                    {
        ...
        private void Populate(DataSet ds)
        {
            ...
            L_obsolete.Text += "<div class='responsive-table-line' style='margin:0px auto;max-width:700px;'><table class='table table-bordered table-condensed table-body-center' ><tbody>" +
                    ...
                    "<tr><td data-title='Numéro de série'>" + ds.Tables[0].Rows[0]["SERIAL_NUMBER"].ToString() + "</td>" +
                    "<tr><td data-title='Statut actuel'>" + ds.Tables[0].Rows[0]["STATUS_TO"].ToString() + "</td></tr></tbody></table></div><hr/>";
